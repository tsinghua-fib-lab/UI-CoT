...
# 数据：RMSE 和 Accuracy
rmse_values = [0.05523, 0.06525, 0.04831, 0.05467]  # RMSE 值
acc_values = [0.6316, 0.5976, 0.7578, 0.6583]   # Accuracy 值

# 设定 2x2 网格的坐标
positions = [(0, 0), (0, 1), (1, 0), (1, 1)]  # 这对应高种族收入比高收入、高低、低高、低低的四个格子

# 计算圆的大小（可以放大一个常数因子来提高视觉效果）
accuracy_sizes = [acc * 50000 for acc in acc_values]

# 创建颜色的归一化对象
norm = plt.Normalize(0.04, 0.07)  # 将颜色映射范围设定为 RMSE 的最小和最大值
...
plt.xlim(-0.5, 1.5)
plt.ylim(-0.5, 1.5)
plt.xticks([0, 1], ['High Gini\nCoefficient', 'Low Gini\nCoefficient'])
plt.yticks([0, 1], ['High\nIncome', 'Low\nIncome'])
#plt.title('RMSE and Accuracy for Different Scenarios')

# 展示图形
plt.tight_layout()
plt.savefig("/data3/maruolong/Train_Data/Add_Path/separate_by_gini_bw_and_income/result_gc/gc_income_accuracy_RMSE_plot.pdf")
plt.show()
