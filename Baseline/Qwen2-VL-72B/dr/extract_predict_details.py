...
def process_jsonl(input_jsonl, output_jsonl):
    ...
    with open(input_jsonl, "r") as infile:
        for line in infile:
            ...
            # 提取predict中的详细信息
            predict_details = extract_predict_details(sample["predict"])

            # 将提取的内容加入到predict字段下
            sample["predict"] = predict_details
...
# 输入和输出文件路径
input_jsonl_path = "/data3/maruolong/Train_Data/change_path_number/dr/result/5_images_dr_CoT_result.jsonl"
output_jsonl_path = "/data3/maruolong/Train_Data/change_path_number/dr/result/5_images_dr_CoT_result_update.jsonl"
