...
def calculate_gini_with_ranges(income_ranges, population_percentages):
    ...
    # 估算每个收入区间的代表性收入
    income_values = [
        (min_income + max_income) / 2 if max_income else min_income * 1.8  # 对高收入无上限区间，用 min_income 的 2 倍
...
def update_jsonl_with_calculate_gini(input_jsonl_path, output_jsonl_path):
    ...
    with open(input_jsonl_path, "r") as infile:
        for line in infile:
            ...
            # 提取收入分布数据
            income_distribution = [
                sample["predict"].get("LEVEL1", 0),
                sample["predict"].get("LEVEL2", 0),
                sample["predict"].get("LEVEL3", 0),
                sample["predict"].get("LEVEL4", 0),
                sample["predict"].get("LEVEL5", 0),
                sample["predict"].get("LEVEL6", 0),
                sample["predict"].get("LEVEL7", 0),
                sample["predict"].get("LEVEL8", 0),
                sample["predict"].get("LEVEL9", 0),
                sample["predict"].get("LEVEL10", 0)
            ]
            ...
            # 添加新的字段
            sample["predict"]["Calculate GINI"] = round(calculate_gini, 2)
...
# 输入和输出文件路径
input_jsonl_path = "/data3/maruolong/Train_Data/Add_Path/Train_result/extract_from_5_CoT_pretrained_data.jsonl"
output_jsonl_path = "/data3/maruolong/Train_Data/Add_Path/Train_result/extract_from_5_CoT_pretrained_data2.jsonl"
